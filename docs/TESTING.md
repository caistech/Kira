...
Passwords live in the password manager / CI secrets — never committed, never pasted into a report.

## One-time provisioning

```bash
# 1. Set the four QA_TEST_* vars in .env.local (from the password manager).
# 2. Create/confirm both accounts (idempotent; sets email_confirm:true so they work even though
#    mailer_autoconfirm is OFF):
node --env-file=.env.local scripts/provision-qa-accounts.mjs
# 3. Ensure ADMIN_EMAILS (Vercel prod+preview AND .env.local) contains QA_TEST_ADMIN_EMAIL
#    and NOT QA_TEST_USER_EMAIL.
```
