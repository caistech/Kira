...
| ID | Item | Type | Sev | Detail |
|---|---|---|---|---|
| **G2** | Per-tenant `from` **and sender identity** resolution | MISS | **1** | **Unblocked by G1 closing — this is now the only thing between a verified F2K domain and F2K mail actually leaving as F2K.** Raised from Sev 2: with the domain live, this is the whole remaining gap. `lib/email/resend.ts:25` hardcodes `Kira <noreply@updates.corporateaisolutions.com>` (`EMAIL_FROM` is set in no environment) and owner-behalf sends — `lib/kira/swarm/stub.ts:281` and `:315` — call that same `sendEmail()`. So a F2K quote goes to a F2K customer from the AI company's domain **with Global Buildtech's ABN in the footer**. Both halves must move together: a per-send `from` on the owner's verified domain **and** `compliance.sender` carrying the owner's entity + ABN. `@caistech/email-send` already supports the override — wiring, not a package change. Kira's *own* product mail (magic link, welcome-back) correctly stays CAS; do not move it. |
