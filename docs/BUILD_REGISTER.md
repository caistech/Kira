...
| # | Item | Verdict |
|---|---|---|
| **J1** | Stripe checkout: no GST on the figure; "per unit / based on usage / Price varies" | DEBT | 2 ⬇ | **HALF FIXED AND OBSERVED 2026-08-03, and the ROOT CAUSE was not what anyone thought.** The tax suffix now renders — verified by screenshot of a real test-mode session: *"Subscribe to Kira Business Plan — Growth (+ GST)"*, and again on the line item. ⚠️ **Why `53babf7` appeared to do nothing for a week:** `ensureMeteredPrice` is idempotent on the lookup key, so `productName`/`productDescription` are used ONLY at Product creation — and Stripe **refuses to update a product it auto-created** (*"The product was created by Stripe automatically and cannot be updated"*, confirmed by trying). The Growth product was created 24 July; the suffix commit landed 3 August; the copy was frozen in between. It was never a copy problem and no amount of editing the string could have fixed it. **The fix is bumping `PRICE_LOOKUP_PREFIX`** (`kira` → `kira-gst`), which mints a fresh Price + Product carrying current copy — documented at the constant, because this is now the ONLY way checkout copy changes. **Still open:** `A$0.00 due today`, `Price varies` ×2 and `per unit` remain, and the productDescription does NOT render (Stripe substitutes its own *"Billed monthly based on usage"* for metered prices). That half needs `custom_text` — which `@caistech/subscription-billing` does not accept, so a package change plus consumer propagation — or Stripe Tax. Severity 1 → 2: the GST omission Ray named is closed; the alarming-but-true metered wording is not. |
...
**Net: one partially open (J1 — GST closed, metered wording remains), four fixed (J2, J4, J5, J8), three unverifiable from source (J3, J6, J7).**
