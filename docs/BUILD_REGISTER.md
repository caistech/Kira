...
| **B13** | The sensitivity filter is a matcher, not a model — recall is its weak side | DEBT | 2 | — | `lib/genome/private.ts` is deterministic, so a paraphrase that avoids every pattern reaches the buyer's copy. It is a mechanism rather than a request, which is the bar, but it is not a model of meaning. Fix is the same shape as `genome_section`: classify once at write time in the existing LLM call (one extra field, no extra call), cache on the row, backfill the ~600 existing rows. **Do not read B7 being closed as this being finished.** |
| **B14** | Facts about the SOFTWARE are filed as `only-you` and reach the buyer's document | BUG | 2 | — | Found while closing B7. The three entries that now travel to the QA buyer's copy are all about Kira, not the business — *"restricts access to all conversation content to only himself and the assistant"*, *"prefers to maintain strict control over communications and approvals"*. The classifier prompt in `derive.ts` explicitly calls this out: *"Anything about the ASSISTANT, the app or the software… This is the most common mistake."* It is happening anyway. A buyer's advisor opens the handover document and reads the vendor's AI-assistant preferences. **Not a same-session close** — needs classifier prompt work plus a re-classification pass over rows already filed, i.e. LLM behaviour and a backfill, which is the same machinery as B13 and should probably be done with it. |
