...
| ~~**O7**~~ | ~~`/about` publicly offers "Try Kira Free"~~ | — | — | ✅ **CLOSED** — recorded in the O-closures table below, and closed a second time by the `/about` rewrite (`64fca4f`) which removed the page those CTAs lived on. `grep -r "Try Kira Free" app/` returns nothing, 2026-08-09. This row kept its OPEN styling in the table while the closure sat twelve lines further down — the same two-places-one-fact shape as **E1**/**K11**. Original below. | `app/about/page.tsx:31` and `:145` — two CTAs reading *"Try Kira Free"* and *"Start Talking to Kira →"*, both pointing at `/start`, on a **public** page, for a product that charges **$999 + GST/month with no trial** (`trialDays` was deliberately removed — arrears is not a trial). Exactly the **J2** class: a free-beta promise that went false the moment `STRIPE_LIVE_MODE` was flipped, still live on a page a podcast or broker link would land on. This is what **K15**'s unbuilt `single-statement` check exists to catch, and it is now the second confirmed instance. |
| ~~**O6**~~ | ~~`/api/onboarding/complete` resets the password of any existing account~~ | — | — | ✅ **CLOSED — and this row was STALE for a day on the most severe item in the file, which is the failure this register exists to prevent.** Verified 2026-08-09 against the code: `updateUserById` appears **nowhere** in `app/api/onboarding/complete/route.ts`. The already-exists branch now mutates nothing and says why at length — deliberately not the valuation upsert either (keyed on `user_id` with `onConflict`, so an unverified caller could overwrite a real owner's baseline) and not `stripe_subscription_id` (it would detach the subscription he is actually paying for). Both are vandalism rather than disclosure, and neither is ours to risk on an unproven identity. **Found stale by an operator asking "are we left with any broken things", not by anyone re-reading the file.** Original below. | Lines 63–75: when `createUser` fails because the email exists, it finds that user and calls `updateUserById(existing.id, { password })` with the password the caller just typed. Stripe never verifies that the email typed at checkout belongs to the payer. Mechanism: pay (arrears ⇒ **$0 due today**, attacker's own card), enter a victim's email, take `session_id` from the success URL, POST a chosen password → the victim's account, and their Genome, is yours. **Not executed — reported as a mechanism, not a confirmed exploit.** On a product whose entire proposition is that an owner tells it things he has not told his staff, this outranks everything else in this section. Secondary: `listUsers({perPage:200})` only searches the first 200 users. | Lines 63–75: when `createUser` fails because the email exists, it finds that user and calls `updateUserById(existing.id, { password })` with the password the caller just typed. Stripe never verifies that the email typed at checkout belongs to the payer. Mechanism: pay (arrears ⇒ **$0 due today**, attacker's own card), enter a victim's email, take `session_id` from the success URL, POST a chosen password → the victim's account, and their Genome, is yours. **Not executed — reported as a mechanism, not a confirmed exploit.** On a product whose entire proposition is that an owner tells it things he has not told his staff, this outranks everything else in this section. Secondary: `listUsers({perPage:200})` only searches the first 200 users. |
...
### Closed 2026-08-09, second pass — the rest of Ray's report

Everything below closed in one session on the operator's instruction to take the remaining list.
The four commits are `87dcd38` (valuation), `642aecc` (landing/copy), `7655d5c` (agents), `0a34bba`
(voice). **Read the "not verified" line on each — several are shipped and not yet observed.**

| ID | What was OBSERVED |
|---|---|
| ~~**P7**~~ | ✅ **ONE more screen, not three more questions.** His framing was "one more screen turns an indicative valuation of the business into roughly what you'd walk away with", and three extra steps would have taken the questionnaire from 12 to 15 in answer to a complaint about incompleteness — with the eleven-in-three-minutes shape being the best-reviewed thing on the site. A `group` step renders **work in progress**, **premises** and **how long he wants to keep running it** on one screen: 12 screens → 13. **Each is treated differently and the differences are the honest part.** WIP is the mirror image of debt — it ADDS to all four figures, so the gap, the multiple, `MODEL_VERSION`, six stored snapshots and his monthly price are untouched — and is stated as *not certain money*, because settlement at completion is a term of the contract rather than a law and a retention on a job he walks away from may never be released. **Premises changes NO number at all**: it buys a disclosure (the property is not in any figure, and a buyer will re-work his profit at market rent), because pricing it needs a market rent we do not have and guessing would corrupt the ONE input the model runs on. **Timeframe changes no number and NEVER LEAVES THE DEVICE** — H3 is do-not-infer-exit, and the handoff is claimed at signup and becomes a permanent row, at which point "wants out in two years" is a fact about a man in a database an agent can read and repeat; `forSharing` strips it, a test pins it, the question says so on screen. `net-of-debt.ts` → `net-position.ts`, because a function called `netOfDebt` that also adds receivables is a name that lies at the call site; options object, since two same-typed money args swap silently and INVERT the answer. 38 tests. **Not verified:** never rendered in a browser — the result view is client-only, thirteen answers in. |
| ~~**P9**~~ + ~~**P10**~~ | ✅ Date, sector, every answer as given, then SDE expanded from the canonical constant. **Derived from the questionnaire, not hand-written** — `record` is a REQUIRED field on the step type, so a fourteenth question cannot be added without deciding what it is called on the document. The invariant under test is "every question asked appears on the record", which is the only version that survives another question. 9 tests. **Not verified in a browser.** |
| ~~**P11**~~ | ✅ "Print, or save as a PDF". Plus a print stylesheet — the gap headline is white-on-violet and was going to print the largest number on the page **white on white**. **Not verified:** no print preview taken. |
| ~~**P17**~~ | ✅ "$460 thousand" → "$460k"; "annual PROFIT" no longer shouts. |
| ~~**P8**~~ | ✅ Each landing number now says what it means, read from `lib/valuation/headline-numbers.ts` — the same constants the valuation intro uses, so four surfaces cannot come to describe three numbers differently. **Observed** in the served HTML. |
| ~~**P13**~~ | ✅ **THE ROW WAS STALE — this was already done.** The `/about` rewrite (`64fca4f`, DECISIONS §8) did exactly what Ray recommended: the landing founder section at full length, site tokens, no emoji, none of the portfolio-deck vocabulary. It also already carried a per-page title. Found by reading the file, not by anyone noticing — the same failure the O6/E1 banners record. |
| ~~**P15**~~ | ✅ **PASSES, behaviourally.** A real question typed into the box fires the POST (network asserted, not presence), and the acknowledgement renders **inline under the control at (232, 586)** rather than as the corner toast a tester called "a different part of the room". It no longer promises a reply it cannot send, and offers a mailto. ⚠️ **Found on the way:** the panel opens carrying Mute and End, zero inputs, for **~9 seconds** before the text box a mic-less visitor needs appears — the defect convai fixed in **0.13.0** ("the eight seconds nobody was told about"). Kira was pinned `^0.12.2`, and a caret on 0.x pins the MINOR, so it could never arrive on its own. Bumped. **Not verified:** the 0.13.0 connecting state — this machine gets a 500 on the signed-URL fetch, so only the FAILURE path was exercised. |
| ~~**P18**~~ | ✅ What is TRUE TODAY now goes first, and the roadmap item is described by the half that is actually the feature — the leaving, not the listening. "Sits quietly in the background… like Siri" is gone; a guard pins the phrasing AND pins that the section cannot be satisfied by deleting it. **Observed** in served HTML: replacement present, both retired sentences return zero. |
| ~~**P19**~~ | ✅ `whitespace-nowrap` is what actually guarantees the nav stops breaking mid-phrase (a wider container only moves the width at which it happens); the header also widens on `xl`. The empty right half is fixed by COMPOSITION — widening the reading measure to fill space would make the page harder to read for a 66-year-old — so at `lg:` the hero is two columns with her portrait on the right. **Not verified in a browser at 1440px.** |
| ~~**P20**~~ | ✅ Fixed as a CLASS: every `@public-route` page declares its own title, no two are the same, and the distinguishing word leads because a tab strip shows ~20 characters. Hung on `@public-route` because two portfolio-gate audits already maintain that marker. **Observed** — six distinct titles served. |

**Three defects found while working, none in any report:** the debt question's help said "leave it blank" while `canAdvance` disabled Next until a number was typed (the copy promised what the button refused, on the question an owner is most likely to decline); the after-debt heading named one of what are now two operations; and "doesn't account for your … working capital" went stale the moment WIP was applied — the P4 class again, code changed, prose beside it did not, nothing errors.

**⚠️ LOCAL AND PRODUCTION SERVE DIFFERENT LANDING PAGES.** `NEXT_PUBLIC_STYLE_NEW` is set in Vercel and not locally, so `next start` here serves **LandingClassic** while kiraexec.com serves **LandingNew**. Every landing observation above was taken against a local build with the flag set. A run that skipped this step would have tested a page no visitor sees — and Ray's report is against LandingNew, which is how this was noticed.

### Open — filed, not fixed

| ID | Item | Type | Sev | Detail |
|---|---|---|---|---|
| ~~**P3**~~ + ~~**K5**~~ | ~~8 of 9 example Genome areas "are not clickable" / `/genome` paints slowly~~ | — | — | ✅ **CLOSED 2026-08-09 by removing the DEPENDENCY rather than shortening the wait — and the measurement that was owed could not honestly be taken.** ⚠️ **I discarded my own timing numbers, which is the part worth keeping.** Sampling production returned **0.3s to 8.6s for the same cached 33KB document** (`x-vercel-cache: HIT` on every request, TTFB consistently 0.25–1.1s) — which looked like a spectacular finding until a control against an **unrelated host** returned the same spread. The variance was this link, not the product. `TESTING_STANDARD` §2.5 exactly, and reporting it would have been worse than not measuring. **So the fix is structural, and that is a strength here rather than a compromise:** the inert window's length is a property of *his* device, which cannot be measured from here, so removing the window beats shortening it. **(1) `/genome` is now a server component.** The accordion was the only interactive thing on the page and it made the whole page `'use client'`, so every card's clickability waited on the bundle. Native `<details>`/`<summary>` is opened by the browser with no JS at all — verified in the built output: **9 `<details>`, exactly one `open`, all nine panels' content present in the SSR'd HTML**, route `○ (Static)`. `name="genome-area"` keeps exclusive-accordion behaviour where supported and degrades to independent panels where it is not. **(2) The webfonts are self-hosted.** Five pages — `/genome`, `/business-valuation`, `/plan`, `/onboarding`, LandingClassic, *every one of them a page he called slow* — loaded fonts with an `@import` of a Google stylesheet **inside a body `<style>`**: render-blocking, invisible to the preload scanner, third-party, no preconnect, and it triggers a second request for the font files. `next/font` now self-hosts both faces; the built HTML contains **zero** references to `fonts.googleapis.com`. ⚠️ **A trap caught by reading the built CSS, not by anything failing:** moving `.font-display`/`.font-body` into `globals.css` would have applied Outfit/DM Sans **site-wide across 256 usages** — `tailwind.config.ts` maps both to Inter and **`DESIGN.md` §4 records that as a deliberate deferral** ("adding a second face mid-testing"). Both compile to the same selector at the same specificity, so it came down to sheet order and mine landed last. The two rules therefore stay page-scoped in each `<style>` block, where a body rule beats the head sheet — the external request is gone, the typography is untouched. `app/no-blocking-fonts.test.ts` guards both halves (5 tests, **mutation-proven** — reinstating one `@import` names the file). Build green, 846 tests, `tsc` clean. **Not observed in a browser.** Original below. | ⚠️ **INVESTIGATED, AND THE REPORTED DIAGNOSIS IS WRONG — the handlers are real and they work.** Measured live on production, asserting `location.hostname` in the same call: 9 cards, 9 buttons, and clicking *"Who does the work"* after network idle expands it **112 → 817 characters** while card 1 collapses. What IS real: **a click issued immediately after the load event is silently dropped** — `aria-expanded` unchanged, no article's text length moves, and **no console errors**. So the page is served, readable, and inert for a window; `/genome` is `'use client'` end to end, so its entire interactivity waits on the bundle. ⚠️ **The gap is stated rather than closed:** the window measured only **~300ms** from a fast headless client on a fast link (goto 1045/1101/1234ms, then network idle +275/369/281ms), which does **not** explain a man clicking, waiting, and concluding it is not clickable. Either his client was far slower or something else is happening, and I have not established which. **This is almost certainly the same root as `K5`** (*"`/genome` paints slowly, cause unknown"*) — treat them as one item. **Candidate cause, found while looking and not yet measured:** six page files inline `@import url('https://fonts.googleapis.com/…')` inside a `<style>` element in the body — `about`, `business-valuation`, `genome`, `onboarding`, `plan`, `LandingClassic` — which is a render-blocking, late-discovered stylesheet on exactly the pages he called slow. `next/font` or a `<link>` in the root layout is the fix if it holds up. **Next step is a measurement, not an edit.** |
| ~~**P4**~~ | ~~The sector's average multiple is referenced and never stated~~ | — | — | ✅ **CLOSED 2026-08-09 — and the sentence he could not check was also FALSE for some owners.** The figure is now printed under the number cards (`lib/valuation/sector-context.ts`), and the **direction is derived from the two figures as printed** rather than asserted in prose. That mattered more than the missing number: *"sits below your sector's average"* lived in the LOW-readiness branch on the assumption that low readiness means below the median, and since the band was sector-scaled on 08-08 that no longer follows — the ceiling carries a size adjustment the floor does not, and the absolute seller's floor (1.5×) sits above a cheap sector's scaled floor. **An owner answering "I am the business" in Routes (1.51× median) on $120k SDE is priced at 1.6× — above his sector — and was told he was being marked below it.** The strong-readiness twin was false the other way: a marina at a 6.6× median tops out at 5.0× on the hard buyer ceiling and was told it was *"near the top of what your sector commands"*, a turn and a half short. ⚠️ **Two further claims on the page described a model that had been replaced**, found while fixing this and closed with it: *"Where the range comes from… The bottom is 1.5×. The top is 5×"* and *"Sector figures inform the commentary, not the number."* Both were true of the **universal band (08-04 → 08-08)** and false the moment sector scaling landed — the second flatly contradicting *"This sets the multiple, so if it is wrong the number is too"* 300px above it. The page now prints **his** band (`floorMultiple`–`ceilingMultiple`, 2.2×–4.2× on Ray's figures against the 1.5×–5× it was showing him) and cites the source by name. **The cause is worth keeping:** the 08-08 change was made in `model.ts` and nobody re-read the page that *explains* `model.ts`. Nothing catches that — the code is correct and the prose beside it has quietly gone stale, so there is no error, no redirect and no blank page. `app/business-valuation/claims.test.ts` now pins it (comments stripped, because its own header quotes the retired sentences); **mutation-proven** — reinstating *"inform the commentary, not the number"* turns it red naming the assertion. `stripComments` was extracted to `lib/source-scan.ts` on its second consumer rather than copied. 12 new tests, `tsc` clean, 187 green across the valuation surface. **Not yet observed in a browser.** Original below. | *"which is why the multiple sits below your sector's average, not at it."* **Below by how much?** His words: *"If electrical contracting averages 4.1× and I'm at 2.8×, that's a fact I'd chew on for a week. As written it's a claim I can't check, on a page whose whole credibility rests on being checkable."* Severity 1 because the number is already in the codebase (`sde-multiples.ts`) — we are withholding the one figure that turns the argument from an assertion into arithmetic, on the page built to be checked. It also directly serves **N5** gate 5 (*the valuation ties to its cited source on screen*) and is far cheaper than anything else in that gate. |
| ~~**P5**~~ | ~~The calculator's uplift is half the landing example's~~ | — | — | ✅ **CLOSED 2026-08-09 — and the premise needed one correction: BOTH figures are the model's.** `landing-example.test.ts` already pinned the page to the calculator (that was built on 08-05 after the 2.25× overstatement), so the landing was not stale. Measured: the plumbing example runs at **readiness 0.08** — every capturable factor at its worst, nothing written down, no contracts, one big customer — and Kira's claim is **proportional to what is left to capture**, so it earns the full 28.5%. The same plumber partly documented (readiness 0.38) gets **14.5%**; Ray's own 15.5% sits exactly there. **So the model is consistent and the presentation was not:** the maximum-headroom case was captioned *"a real plumbing business, run through the actual calculator"*, which is true and reads as typical. Fixed by naming the case on both landings — *"That is the widest the gap gets. An owner who has already documented half of it sees roughly half as much, because Kira only claims what is left to capture"* — **not** by re-inflating the model, which would undo A1–A4. The qualifier is a claim about the model, so it is **checked against the model**: three new tests assert the example is the maximum-headroom business, that a half-done owner is quoted 0.35–0.65× the gain, and — the load-bearing one — that more work already done always produces a **smaller** gap. **Mutation-proven**: making the comparison business identical to the example turns two of them red. ⚠️ **This turns the complaint into a credibility gain and that is the point** — a reader's smaller number now reads as the model being consistent rather than as the shop window overselling the shop. Original below. | His result moved 2.8× → 3.2×, **15.5%**. The plumbing example on the landing goes $684k → $879k, **28.5%**. *"the marketing example is nearly twice as flattering as what the calculator gave me."* Worse in combination with the product's own language: he is told *"sell an asset, not a job"*, and the tool then puts him at 3.2× against a stated range topping at 5×, i.e. still selling a job after doing all the work. ⚠️ **This is the A1–A4 class arriving from the other end.** Those were fixed by claiming LESS; the landing example was not re-derived at the same time, so the marketing number now overstates what the corrected model produces. `landing-example.test.ts` exists — check whether it pins the example to the model or merely to itself. **Do not fix by re-inflating the model.** |
...
| ~~**P7a**~~ | ~~Debt — the number he actually cares about~~ | — | — | ✅ **CLOSED 2026-08-09, absorbing `K7`.** Question 12, *"Roughly what does the business owe?"*, and the result page now carries an **after-debt block**: worth today, with-knowledge-captured, and if-you-closed-up, all net. The disclaimer that used to set him homework — *"Subtract any loans, equipment finance, lease obligations or tax owing"* — is gone; that was the product asking a 66-year-old to do arithmetic it had every input for except one, and never asked for. **The gap is provably unmoved**, so the multiple, `MODEL_VERSION`, the six stored snapshots and his monthly price are all untouched: debt comes off `today` and `potential` equally and `model.ts` never reads the field (it is optional, per **C5**'s failure mode). **Walk-away is NOT floored at zero** (operator: *"better for him to be clear and walk away than promise something from a not real basis"*) — Ray's own figures render **−$308,000 – −$272,000**, with a sentence saying closing up is not available to him, which fires only when even the TOP of the range is under water so it states a fact rather than a possibility. **Precondition closed first:** the interest add-back was in `SDE_DEFINITION` but missing from the **worked example** and the **confirmation note** — the two surfaces an owner copies — and `model.ts`'s own field comment gave a third, shorter definition. That mattered the moment debt was subtracted: SDE is pre-debt-service, so an owner omitting interest hands us a figure that already absorbed his debt service and we then take the principal off again, both errors pushing the same way. ⚠️ **A real defect was caught ON SCREEN after every unit test was green:** the first version subtracted debt from the RAW pair and rendered **$712,000 and $988,000 under a headline gap of $280,000** — 988 − 712 = 276, four thousand out, in the largest type on the page. Exactly **J8** returning, and on the page whose own paragraph invites that scrutiny. Fixed structurally rather than carefully by feeding the **displayed** pair, so the subtraction closes by construction; pinned by a test over deliberately awkward rounding cases. **Observed live** at 1440px on the built app with Ray's figures: $710,000 / $990,000 / −$308,000 – −$272,000 against a $280,000 headline — reconciles exactly. 22 tests, mutation-proven (9 fail without the subtraction). Incidental corroboration for **C5**: seeding invalid enum values produced **`$NaN`** on screen rather than a rejection. |
| **P7** | The other three: premises, WIP, when he wants out | MISS | 2 | **Debt** (~$380k of vehicle and equipment finance — currently a disclaimer at the bottom telling him to do the arithmetic himself), **the premises** (he owns the yard through his super fund; for a trade business often the biggest single question in the deal), **WIP and retentions** (real money on other people's balance sheets), and **when he wants out** (*"the entire product is aimed at a man in his sixties and never once asks how long he's got. Two years and eight years are completely different advice"*). His framing is the product one: one more screen turns *"an indicative valuation of the business"* into *"roughly what you'd walk away with"* — *"That's the number I'd screenshot and show my wife."* ⚠️ **Debt is already filed as `K7`** — merge rather than duplicate. ⚠️ *When he wants out* runs straight into **H3**'s do-not-infer-exit rule: asking is fine, inferring is not. |
...
| ~~**P12**~~ | ~~`/plan` server-renders a loading message~~ | — | — | ✅ **CLOSED 2026-08-09 — the money question is now answered by the SERVER.** The pre-hydration branch used to end on *"Reading your valuation from this device…"*; it now states, from the constants that actually charge him, **the floor price, the arrears basis and the 12-month cap**. **Observed on the real server response** (`next start`, `curl`, tags stripped): **606 visible characters outside the chrome**, containing *"From $499 + GST a month"*, *"billed after each month has finished, never in advance — cancel before then and that month is on us"* and *"After 12 months you move to a third of the rate"*; the old loading line returns **0**. ⚠️ **His personalised figure genuinely cannot be server-rendered** and that is deliberate, not laziness — it derives from turnover and profit held in `sessionStorage` precisely so the numbers never enter a URL, a `Referer` header or browser history. So the fix is not "render it sooner", it is *answer the question that does not need him* first and let the personal figure refine it. ⚠️ **The floor only, never the band table** — `pricing.ts:35–39` records that as a decision (2026-08-01): the floor answers *"roughly what does this cost"*, a table invites band-shopping before there is a gap to size it against. `app/plan/first-paint.test.ts` pins **both** directions (5 tests). **The reason no gate caught this is worth keeping:** `first-paint` **passes** `/plan` and is right to — there is real text — `public-routes` passes, `deploy-status` passes. The defect was never whether the page says something or how long the wait is; it is *what it says while it waits*, on the one page where saying nothing about money costs the sale. Original below. | The page behind *"Start building your Business Genome"* — **the page where money is asked for** — arrives from the server as a heading, one paragraph and *"Reading your valuation from this device…"*; the price is drawn afterwards by the browser. He explicitly does **not** claim it is slow (his browser died before he could time it): *"of every page on this site, the one that starts as a loading message is the one where I'm deciding to pay you."* Note `portfolio-gate-audit-first-paint` **passes** `/plan` — correctly, since there is real text — so this is a gap the existing check cannot see by design. Same neighbourhood as **P3**/**K5**. |
...
| ~~**P14**~~ | ~~`/about` contradicts the landing page, twice~~ | — | — | ✅ **CLOSED 2026-08-09 with the `/about` rewrite (`64fca4f`)** — both sentences went with the page. **Verified against PRODUCTION HTML, not the source:** `"Talk, don't type"` **0**, `"gets better with every conversation"` **0**, `"Longtail"` **0** on `https://kiraexec.com/about`. ⚠️ Grepping the SOURCE returns 1 for each, because the rewrite's header comment quotes the removed strings to record what went and why — the same false positive `site-chrome.test.ts` hit on its own explanation, and the reason that test strips comments. A source grep here reads as "still broken" and is wrong. Original text below. | `/about` said *"Voice-First — Designed for natural conversation. **Talk, don't type**"* against the landing's *"if you would rather type, you can"* — to the one persona who will never speak in an office. And *"Always Improving — Every agent learns, adapts, and gets better with every conversation"*, which he read as *"you train on what I tell you"*, against the landing's explicit *"not sold, not pooled, not used to train anyone's model."* *"I know you probably mean two different things. I can't tell which one to believe."* Third confirmed instance of the class **K15**'s unbuilt `single-statement` check exists to catch. |
...
| ~~**Q3**~~ | ~~The Gmail access levels are a type, a component nobody renders, and 9 passing tests~~ | MISS | 2 | ✅ **CLOSED 2026-08-15 (`d74b499`)** — and closing it surfaced a worse one sitting underneath. **The Gmail level is now chosen by the owner** on `/setup/drive` (`none`/`draft`/`read`, defaulting to **`none`** — a mailbox is never pre-ticked, and `google-connect.ts` argues the same on the other side of the seam), read by `actions.ts` and passed into the signed claim. `ConnectChoices` is **deleted**; its writing was good, so the gain/cost shape moved into the mounted `DriveConnectForm` rather than being rewritten. **The new guard asserts the WIRING, not only the copy** (`components/drive-connect-form.test.tsx`, 18 tests, three mutation-verified) — that the page imports the form, that the action reads `gmail` off the FormData, and that it reaches `googleConnectLink`. The nine tests it replaces could not have failed, because a content test cannot see an unreachable feature; that is the whole lesson of this row. ⚠️ **THE WORSE ONE: contacts were requested on every connection and disclosed on none.** `scopesFor()` spreads `CONTACTS_SCOPES` in unconditionally — including `contacts.other.readonly`, every address Google auto-saved from mail he has sent — and `/api/connect/google` verifies the ticket and redirects **straight to Google with no interstitial**, so this page was his ONLY description of what he was granting and it said "Let Kira read your Drive". He pressed Continue expecting Drive and Google asked for his address book. For an owner thinking about selling who has told neither staff nor family, that is the most sensitive list in the account. Now stated on the form and in the page header as unconditional. **Read-only also stated only its upside** — "she cannot edit, move or delete anything" is the same fact as "she can never file a finished handover pack back into your Drive", and that half was nowhere on screen. **Recommendation stays `readonly`, deliberately:** `drive.file` cannot open a file the app did not create, so a `picked` connection cannot read the twenty quotes this page's promise depends on. `picked` remains right as the orchestrator's *fallback for an unrecognised ticket*; they are different questions and both answers stand. **Still owed, on the orchestrator side:** `permissionSummary()`/`connectionBlurb()` derive the can/cannot list from the scopes with a test that reddens when a scope moves and the copy does not — and are consumed by **one test script**. `docs/BRIEF_ORCHESTRATOR_CONSENT_SUMMARY.md` asks for `GET /api/connect/summary` so Kira renders derived claims instead of mirroring them here and letting them drift. Until it lands Kira shows only claims true of every combination plus the per-level copy — fewer claims, not invented ones. ⚠️ **Offering `readonly`/`full`/`gmail.read` at all is what triggers CASA** per `BRIEF_ORCHESTRATOR_DRIVE_FILE_SCOPES.md`. All six are offered because all six exist and the owner was being denied a choice the system supports; cutting to `picked`+`draft` is one array in each of two files if the verification decision lands the other way. **Dennis's call, before filing.** Original entry below. | Found by the **orchestrator session** 2026-08-15 while reviewing `af33bbc`, and verified here. `GmailAccess = 'none' \| 'draft' \| 'read'` exists on `ConnectClaim`; `ConnectChoices.tsx` presents all six Drive×Gmail levels with the cost of each stated; `connect-choices.test.tsx` passes 9 tests. **None of it is reachable.** `grep ConnectChoices app components` returns only the file and its own test — **it is imported by nothing**. And `app/setup/drive/actions.ts` is the only caller of `googleConnectLink`, passing `tenantId`, `access`, `email`, `returnTo` and **never `gmail`** — so with `gmail` optional and absent meaning `'none'` on the far side (deliberate, and right), **every ticket minted today reaches the orchestrator as `gmail: 'none'`** and Gmail drafts cannot work whatever an owner picks. He cannot pick anything regardless; there is no screen. ⚠️ **`af33bbc`'s message overclaims it** — *"ConnectChoices puts the six levels in front of him"* is true of the file and false of the product; a banner at the top of the component records that rather than quietly correcting it. ⚠️ **This is the fourth instance this week of the same class** — alongside the readiness score that never recomputes, the genome that was never seeded, and `ensureTrial` which nothing calls. Each is a correct, tested, unreachable implementation, and in every case the tests passed. **Whatever check catches this class is worth more than the four fixes**: a component with tests and no importer is statically detectable, and so is an optional field on a claim type that no caller ever sets. Closing needs a change to `actions.ts` plus a surface that renders the component — not an edit to `ConnectChoices`, which is correct as written. |
