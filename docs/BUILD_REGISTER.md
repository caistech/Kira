...
The monthly rate is *"a dynamic based on a percentage of that gap"* (`priceForGap`, live) — so a model
that inflates the gap **overcharges**, and a broker who discounts the valuation is also disputing the
invoice. That raises A7 from a scoring question to a billing one: re-weighting changes what existing
owners would have been quoted. ⚠️ A **second, incompatible** pricing model was also described in the
same meeting — *"like the budget airline: a low charge to get in, but the add-ons are enormous"*
(voice humanisation, specialist agents). Gap-proportional pricing scales the base with the size of
the problem; the add-on model deliberately keeps the base small. They can be combined (small base +
gap-linked add-ons) but the result page quotes **one** figure derived from the gap, and the landing
page now publishes a **$499 + GST floor** that reversed a documented no-price rule. Reversing it
again shortly after is its own kind of expensive. **Not a decision to take from a transcript — flag
before A7.**
...
| **C6** | Two onboarding interviews are being built in parallel | DEC | 2 | — | ⚠️ **Coordination, and it wants settling BEFORE C3 is built** *(team meeting, 2026-08-02)*. Gareth's action item is *"build an interview skill into the advisor agent… they'll interview for all the right things, and then file into all the correct memory files."* That is the same job as **C3 + Block 0** — a structured onboarding interview that seeds the record. Ours is decided in `GENOME_BUYER_FORMAT.md` §4.1 (origin → baseline → scope → depth) and is gated on B2; his lands in the swarm's memory files. Building both produces either a duplicate interrogation of a 66-year-old on day one, or two records that disagree. Decide which layer owns the interview and which consumes it. |
