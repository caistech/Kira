...
| **E9** | **C1** — a clean signup reaching a baseline | **1** | anyone with a fresh email | Sign up with no prior valuation → the `NoBaselineYet` card appears → *"Answer the eleven questions"* → finish → *"That's mine — use it"* → **both** a `business_valuations` row and a baseline snapshot exist. Built + typechecked + built for production 2026-08-02; the round trip has never been walked. The half most likely to be wrong is the return leg, because it crosses a static page, the device store and `UserShell`. |
...
4. **C1** give a direct signup a valuation path — 🟡 **built, awaiting the walk-through**
