...
| # | Item | Verdict |
|---|---|---|
| **J1** | Stripe checkout shows no GST; "per unit / based on usage / Price varies" | ❌ **OPEN — and the handoff's diagnosis is right.** `checkout/route.ts:108` puts `taxSuffix()` in the **product NAME** only; there is no `custom_text` and no Stripe Tax. `meterEventName` (line 92) confirms a **metered** price, which is exactly what makes Stripe render "per unit / based on usage". Ray reads the figure, not the line-item title. Structural, already attempted once as copy (`53babf7`). |
...
| **J4** | Example Genome has 6 sections, the real one has 9 | ❌ **OPEN — confirmed.** `areas.test.ts` asserts `GENOME_AREAS` has **9**; `lib/genome/example.ts` carries **6** keys (demand, pricing, operations, cash, customers, compliance). People, Assets and Management & records are absent. Same class as **B8**: the public example asserts a shape the product no longer has. |
| **J5** | Kira Exec claims it excludes test accounts, then lists two | ❌ **OPEN — confirmed, and the claim is unsupported.** The on-screen sentence *"This view excludes test accounts and personal Kiras"* and the comment at `lib/admin/exec.ts:5` both assert exclusion; the query has **no such filter** — only `.not('user_id','is',null)` and `.neq('status','deleted')`. Either filter the QA identities or delete the sentence, because a false claim on an operator screen is worse than no claim. |
...
| **J8** | Raw export $1,094,292 vs $1,090,000 everywhere else | ❌ **OPEN — confirmed.** The markdown export rounds via `formatMoneyApprox` (lines 188-189), but the JSON spreads `...g` verbatim, so `worthToday`/`gap` leave **unrounded** in the same file. The product's most persuasive paragraph explains why it rounds; the attachment then contradicts it to the dollar. |

**Net: FOUR confirmed open (J1, J4, J5, J8), one fixed (J2), three unverifiable from source (J3, J6, J7).**
