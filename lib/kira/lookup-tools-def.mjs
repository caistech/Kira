...
import { withSpeakingTo } from './speaking-to-def.mjs';
...
export const KIRA_KEEP_DOCUMENT_WEBHOOK_PATH = '/api/kira/webhooks/keep_document';

/**
 * keep_document — the yes half of an offer she makes herself.
 *
 * Deliberately a SEPARATE tool from read_document rather than a flag on it. A flag is something a
 * model sets; a separate call is something it has to decide to make after hearing an answer, which
 * is the behaviour being encoded: read freely, ask before you file.
 *
 * @param {string} baseUrl
 * @param {Record<string,string>} [headers]
 */
export function kiraKeepDocumentToolDef(baseUrl, headers) {
  const hdrs = headers || { 'Content-Type': 'application/json' };
  return {
    type: 'webhook',
    name: 'keep_document',
    description:
      "Call this ONLY after you have offered to keep a document and the owner has said yes. It files that Drive document into what you permanently know, so you can answer from it in future conversations without going back to Drive. Pass the same `id` you used for read_document. NEVER call it unprompted: reading a document is a question, keeping it is a decision about what belongs in their business record, and that decision is theirs. If the response says already=true you already had it and nothing was duplicated — just say so. If chunks is 0 the document was saved but had no readable text, so tell them you won't be able to answer from it. If ok=false, nothing was saved; say the message as written rather than implying it was kept.",
    webhook: { url: `${baseUrl}${KIRA_KEEP_DOCUMENT_WEBHOOK_PATH}`, method: 'POST', headers: hdrs },
    parameters: {
      type: 'object',
      properties: {
        file_id: {
          type: 'string',
          description: 'The `id` of the file, the same one passed to read_document.',
        },
      },
      required: ['file_id'],
    },
