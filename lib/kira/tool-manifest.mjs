 ...
 */
/**
 * Two of the package's five conversation tools are the SERVER'S bookkeeping, not her decisions.
 *
 * `save_message` asks her to file each turn as it happens, and `update_conversation_topic` asks her
 * to declare mid-call what the call is about. The post-call webhook already receives the whole
 * transcript and distils it, so both ask the model to do the route's filing — and it shows in the
 * data: `kira_messages` held 52 rows against 783 conversations, roughly one saved message per
 * fifteen calls, while the memory that actually works (`kira_memory`, 288 rows) is written by
 * `save_memory` and the post-call distil.
 *
 * The cost of carrying them is not storage, it is ATTENTION. Every tool is an entry in the
 * function-calling menu of a small model on a long call — the exact case where `gpt-4o-mini` was
 * measured dropping tool calls, which is why the portfolio default moved to gpt-4.1-mini. Fewer,
 * sharper tools is the same argument as the trigger-worded descriptions above.
 *
 * `get_conversation_context` STAYS. The typed transport replays its own history natively; a voice
 * call has no equivalent, so she must pull at turn zero. That asymmetry is why this list is not
 * simply the typed transport's — `text-tools.ts` excludes all three for the same reason it excludes
 * these two, and is right on its own transport.
 */
const SERVER_SIDE_BOOKKEEPING = ['save_message', 'update_conversation_topic'];

export function toolDefsFor(journeyType, appUrl) {
  const tools = [
    ...withEntityClassification(
      createConversationTools(appUrl, '/api/kira/webhooks', { platformIdentity: true })
    ).filter((t) => !SERVER_SIDE_BOOKKEEPING.includes(t?.name)),
    ...
    ...(journeyType === 'business'
      ? [
          ...
          // THE AGENDA. Without it she talks about whatever he raised last, which is always the live
          // job — measured on the operator's own account: 96 memories, zero answers to the questions
          // a buyer asks about Customers, 28 Operations entries answering none, `people` empty.
          // Business journey only: the nine areas describe a business, and a personal-journey coach
          // has none.
          kiraAreaAgendaToolDef(appUrl),
          // The end of the migration: the manual leaves us and lands in HIS storage. Business
          // journey only — a personal-journey coach has no operating manual to file, and a tool
          // that writes into someone's document store is not one to hand out by default.
          kiraFileManualToolDef(appUrl),
          // Practice Intelligence. Business journey only: this is commercial research into another
          // organisation, which has no place in a personal-journey coach's tool set.
          kiraResearchOrganisationToolDef(appUrl),
        ]
      : []),
  ];
  ...
  // The caller-carried identity channel (Scope D / two-identifier seam).
  //
  // `get_conversation_context` is the only tool that must know WHOSE context to return at turn zero,
  // before any conversation row exists to bind to. Today that identity comes from a server-baked
  // `?uid` — the agent's OWNER at provision time — which collapses every user of a shared
  // organisation agent onto the owner. The webhook has no auth cookie, so the caller's person_id is
  // carried per-session instead: the page hands it to the VoiceWidget, which sends it as the
  // `user_id` dynamic variable at conversation start (widget-logic.js `buildStartOptions`), and this
  // tool param, filled by the PLATFORM (not the LLM), carries it into the webhook body.
  //
  // Purposely NOT required: a caller that sets no `user_id` variable falls back to the baked `?uid`
  // binding on the server, so nothing that already works stops working.
  const contextTool = tools.find((t) => t?.name === 'get_conversation_context');
  if (contextTool?.parameters?.properties) {
    contextTool.parameters.properties.user_id = { type: 'string', dynamic_variable: 'user_id' };
  }
...
⚠️ NEVER SAY A TOOL'S NAME OUT LOUD. Those names are ours; he has never heard one. "I don't do that
through dispatch" tells him nothing except that you are describing yourself. Name the THING you
cannot do, never the mechanism.`;
}
