...
export function kiraKnowledgeToolDef(baseUrl, headers) {
  ...
  return {
    ...
    // Only `query` — identity is baked into the URL as ?uid at provision (the agent never passes it).
    // A conversation_id param used to be here; the LLM filled it with junk ("current"/"default"), so
    // it was removed to stop wasting the model's attention on a value the handler ignores.
    parameters: {
      type: 'object',
      properties: {
        query: {
