...
// SINGLE SOURCE of the research_practice ConvAI tool shape.
//
...
export const KIRA_RESEARCH_PRACTICE_WEBHOOK_PATH = '/api/kira/webhooks/research_practice';

/**
 * @param {string} baseUrl
 * @param {Record<string,string>} [headers]
 */
export function kiraResearchPracticeToolDef(baseUrl, headers) {
  const hdrs = headers || { 'Content-Type': 'application/json' };
  return {
    type: 'webhook',
    name: 'research_practice',
    description:
      "Gather publicly observable evidence about ONE named healthcare practice — its website, whether it uses an online booking system such as Healthengine or HotDoc, the services and hours it publishes, and any people it names publicly. " +
    ...
    webhook: { url: `${baseUrl}${KIRA_RESEARCH_PRACTICE_WEBHOOK_PATH}`, method: 'POST', headers: hdrs },
    parameters: {
      type: 'object',
      properties: {
        practice: {
          type: 'string',
          description:
            'The name of the practice, as the owner said it — e.g. "Joondalup Family Medical Centre".',
        },
        location: {
          type: 'string',
          description:
