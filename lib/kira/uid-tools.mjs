...
export const UID_TOOL_NAMES = [
  ...
  'facts_to_confirm',
  'confirm_fact',
  // Writes documents into a named person's own storage, so whose account it is could not be
  // anywhere the model can reach. Omitted here it would provision without `?uid` and refuse every
  // call — which is the regression the note at the top of this file describes.
  'file_manual',
  // Practice Intelligence. It reads only public pages, so identity is not protecting anyone's
  // confidential data here — it is present because a research pass SPENDS REAL MONEY (a search API
  // call plus two model calls), and a spend with no owner attached is one nobody can account for.
  // Registering it now also means the identity is already flowing when a later phase persists
  // findings, rather than being added afterwards — which is exactly the omission this file warns
  // about, and it is cheaper to be right the first time than to debug "No user identity" later.
  'research_practice',
];
