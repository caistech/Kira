...
export function kiraDispatchToolDef(baseUrl, headers) {
  ...
  return {
    ...
    name: 'dispatch_task',
    description:
      "When the owner asks you to actually DO something — draft a quote, write a follow-up email to a client, set a reminder — call this to prepare it. It drafts the thing but does NOT send it. Read the returned summary back to the owner and ask if you should send/set it; then call approve_task. IMPORTANT: if the response has needs_recipient_email=true, you do NOT have an email address for the recipient — ask the owner for it (e.g. \"what's Dave's email and I'll send it?\") and pass it to approve_task as recipient_email. Do NOT call this for general questions or advice — only when there is a concrete task to prepare. If it comes back 'unsupported', tell the owner you've noted it and can't do that one yourself yet.",
...
export function kiraApproveToolDef(baseUrl, headers) {
  ...
  return {
    ...
    name: 'approve_task',
    description:
      "Call this ONLY after the owner has heard the drafted task (from dispatch_task) and clearly said to go ahead. Pass the task_id you got back from dispatch_task and approve=true. For an email or quote, if you had to ask the owner for the recipient's email address, pass it as recipient_email — otherwise the send can't complete. If they say no or want changes, do NOT call this with approve=true. Nothing is sent until you call this with approve=true.",
