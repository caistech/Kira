          ...
          <section className="py-16">
            <div className="grad-genome rounded-3xl p-8 sm:p-10 text-white text-center shadow-lg">
              ...
              <p className="text-white/90 max-w-lg mx-auto mt-4 leading-relaxed">
                Kira is <span className="font-bold">{money(model.quote.monthly)}/month</span> (your first month is free) — about <span className="font-bold">{model.quote.fractionOfGapPct}</span> a year of what you stand to unlock. It&apos;s the fractional exec you could never justify hiring, at a fraction of the cost — plus the time, the calm and the handover you can&apos;t put a number on.
              </p>
            </div>
            ...
            <div className="mt-8 bg-white rounded-3xl p-8 border-2 border-violet-200 shadow-sm max-w-lg mx-auto text-center">
              ...
              <p className="text-sm text-stone-500 mt-1">Free for your first month. Then {money(model.quote.monthly)}/month — cancel anytime.</p>
              <ul className="text-left space-y-2.5 my-6 text-stone-700">
                {[
                  'A full month free — nothing charged today',
              ...
              >
                {loading ? <><Loader2 className="h-5 w-5 animate-spin" /> Starting…</> : <>Start free — first month <ArrowRight className="h-5 w-5" /></>}
              </button>
              {error && <p className="text-rose-600 text-sm mt-3">{error}</p>}
              <p className="text-xs text-stone-400 mt-3">
                Secure checkout by Stripe · billed by Corporate AI Solutions. Your card is saved today but nothing is charged. The first payment of {money(model.quote.monthly)} comes out a month from now, and we email you three days before. Cancel before then and you pay nothing. You set your password and meet Kira right after.
              </p>
            </div>
          </section>
